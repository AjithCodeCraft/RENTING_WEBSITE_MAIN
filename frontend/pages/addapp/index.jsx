          ...
          <CardContent>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div>
                ...
                <Input id="rent" type="number" {...register("rent", { valueAsNumber: true })} placeholder="Enter rent amount" />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="longitude" type="number" {...register("longitude", { valueAsNumber: true })} placeholder="Enter longitude" />
                  {errors.longitude && <p className="text-sm text-red-500">{errors.longitude.message}</p>}
                </div>
                <div>
                  <Label htmlFor="latitude">Latitude</Label>
                  <Input id="latitude" type="number" {...register("latitude", { valueAsNumber: true })} placeholder="Enter latitude" />
                ...
                <div className="flex flex-wrap gap-4">
                  {images.map((image, index) => (
                    <div key={index} className="relative w-24 h-24">
                      <img src={URL.createObjectURL(image)} alt={`Hostel Image ${index + 1}`} className="w-full h-full object-cover rounded-lg" />
                      <Button variant="ghost" size="icon" className="absolute top-0 right-0 p-1" onClick={() => removeImage(index)}>
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
                  ))}
                  <label htmlFor="imageUpload" className="w-24 h-24 border-2 border-dashed rounded-lg flex items-center justify-center cursor-pointer">
